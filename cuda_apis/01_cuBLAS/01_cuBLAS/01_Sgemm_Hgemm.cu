...
int main() {
    ...
    // cuBLAS SGEMM (single precision 32-bit floating point)
    float alpha = 1.0f, beta = 0.0f;

    // SGEMM - C = \alpha * op(A) @ op(B) + \beta * C
    // Set \beta = 0.0 for simple matrix multiplication
    // op() is a specific operation applied to matrices A or B before matrix multiplication
    // CUBLAS_OP_N: non-transpose operation (no operation is applied)
    // CUBLAS_OP_T: transpose operation
    // CUBLAS_OP_C: conjugate transpose operation

    // SGEMM expects column major matrices, so the shaping when performing SGEMM with matrix dimensions has to be changed
    ...
    // cuBLAS HGEMM (half precision 16-bit floating point)
    ...
    // Copy the result back to the host and convert to float
    half C_h[M * N];
