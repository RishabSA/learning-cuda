...
// When running this file, to compile with linked cuBLAS and cuBLASLt functions, add:
// -lcublas -lcublasLt -lcuda
...
int main() {
    ...
    // Manually define the input matrices
    float h_A[M * K] = {
        1.0f, 2.0f, 3.0f, 4.0f,
        5.0f, 6.0f, 7.0f, 8.0f,
    ...
    // Allocate device memory or FP32
    ...
    // Allocate device memroy for FP16
    ...
    // Set up matrix descriptors for FP32 by allocating the memory needed to hold the opaque structure
    cublasLtMatrixLayout_t matA_fp32, matB_fp32, matC_fp32;

    // The data precision type specified in cublasLtMatrixLayoutCreate can be either real or complex and have a certain number of floating-point or integer bits
    // cublasLtMatrixLayoutCreate expects column-major matrix dimensions

    // CUDA_R_32F specifies that the data precision is real, 32-bit floating point values
    ...
    // Set up matrix descriptors for FP16 by allocating the memory needed to hold the opaque structure
    cublasLtMatrixLayout_t matA_fp16, matB_fp16, matC_fp16;

    // CUDA_R_32F specifies that the data precision is real, 16-bit floating point values
    ...
    // The data precision type specified in cublasLtMatmulDescCreate can be either real or complex and have a certain number of floating-point or integer bits
    ...
    // Set up matrix operation for A and B
    ...
    for (int i = 0; i < M * N; ++i) {
        ...
        // Uses an increased tolerance for FP16
