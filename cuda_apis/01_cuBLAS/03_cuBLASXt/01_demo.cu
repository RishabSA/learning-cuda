...
#define CHECK_CUBLAS(call) { \
    ...
    if (err != CUBLAS_STATUS_SUCCESS) { \
        std::cerr << "Error in " << #call << ", line " << __LINE__ << std::endl; exit(1); \
    } \
}
...
int main() {
    ...
    // cuBLASXt allows for multiple GPUs, so we have to select the GPU to use
    int devices[1] = {0};
    CHECK_CUBLAS(cublasXtDeviceSelect(handle, 1, devices));

    // Warmup run

    // cuBLASXt manages all host and device memory, so cudaMemcpy does not need to be used
