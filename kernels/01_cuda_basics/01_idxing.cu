...
__global__ void whoami(void) {
    ...
    blockDim: 3-component vector that specifies the dimensions of the block
...
int main() {
    const int b_x = 2, b_y = 3, b_z = 4; // grid dimensions (24 blocks)
    const int t_x = 4, t_y = 4, t_z = 4; // block dimensions (64 threads per block)
    // 2 warps of 32 threads in each block
    ...
    dim3 blocksPerGrid(b_x, b_y, b_z); // 3D cube of shape 2 x 3 x 4 = 24 blocks in a grid (grid size)
    dim3 threadsPerBlock(t_x, t_y, t_z); // 3D cube of shape 4 x 4 x 4 = 64 threads per block (block size)
